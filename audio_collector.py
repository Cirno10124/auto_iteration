...
def collect_audio(chunk_duration, output_dir, sample_rate, channels):
    ...
    while not should_exit:
        try:
            ...
            # 录制音频
            try:
                recording = sd.rec(int(chunk_duration * sample_rate), samplerate=sample_rate, channels=channels)
                sd.wait()
            except Exception as e:
                print(f"错误：录制音频失败: {e}")
                print("跳过当前片段，继续下一个...")
            ...
            # 写入主音频文件
            try:
                wavfile.write(filename, sample_rate, (recording * np.iinfo(np.int16).max).astype(np.int16))
            ...
            if sep_pipeline:
                try:
                    ...
                    for channel, audio in separated.items():
                        ...
                        sep_filename = os.path.join(sep_dir, f"{index:02d}_sep{channel}.wav")
                        wavfile.write(sep_filename, sample_rate, (audio.numpy() * np.iinfo(np.int16).max).astype(np.int16))
                except Exception as e:
                    print(f"警告：分离失败: {e}")
            ...
            try:
                for turn, _, speaker in diarization.itertracks(yield_label=True):
                    speaker_folder = os.path.join(output_dir, f"speaker_{int(speaker)+1:02d}")
                    try:
                    ...
                    segment_filename = os.path.join(speaker_folder, f"{count:02d}.wav")
                    try:
                        wavfile.write(segment_filename, sample_rate, (segment * np.iinfo(np.int16).max).astype(np.int16))
                    except (IOError, OSError) as e:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Microphone audio collector')
    parser.add_argument('--chunk_duration', type=int, default=30, help='Chunk duration in seconds')
    parser.add_argument('--output_dir', type=str, default='audio_chunks', help='Directory to save audio chunks')
    parser.add_argument('--sample_rate', type=int, default=16000, help='Audio sample rate')
    parser.add_argument('--channels', type=int, default=1, help='Number of audio channels')
    args = parser.parse_args()
    collect_audio(args.chunk_duration, args.output_dir, args.sample_rate, args.channels)
