...
class EndToEndTest(unittest.TestCase):
    ...
    def test_full_pipeline_once(self):
        ...
        self._cleanup_outputs(tests_dir)
        ...
        cmd = [
            ...
            "iteration.once=true",
            "iteration.skip_labeling=false",
            "iteration.skip_manifest=false",
            "iteration.stop_after_manifests=true",
            "labeling.model_name_or_path=DUMMY_BASE_MODEL",
            "training.model_name_or_path=DUMMY_BASE_MODEL",
            "labeling.device=-1",
        ]
        ...
        self.assertTrue(
            os.path.exists(os.path.join(tests_dir, "labels", "_labeler_calls.txt")),
            "未生成标注调用记录",
        )
        # 清理测试生成的目录
        self._cleanup_outputs(tests_dir)
    ...
    def test_two_iterations_use_previous_best_model(self):
        ...
        self.assertIn(best_model_dir, calls)
        self.assertTrue(
            any(c in (base_model, best_model_dir) for c in calls),
            f"训练调用记录异常: {calls}",
        )
        ...
        self.assertIn(best_model_dir, lcalls)
        self.assertTrue(
            any(c in (base_model, best_model_dir) for c in lcalls),
            f"标注调用记录异常: {lcalls}",
        )
