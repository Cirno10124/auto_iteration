...
def main():
    ...
    parser.add_argument('--labels_dir', type=str, required=True, help='标注文件输出目录')
    ...
    parser.add_argument('--device', type=int, default=(0 if torch.cuda.is_available() else -1), help='运行设备，-1 表示 CPU，>=0 表示 GPU 设备编号')
    parser.add_argument('--chunk_length_s', type=float, default=30.0, help='长音频分段长度（秒）')
    parser.add_argument('--no_speech_threshold', type=float, default=0.6, help='跳过无语音样本阈值')
    parser.add_argument('--compression_ratio_threshold', type=float, default=1.35, help='跳过高压缩率样本阈值')
    parser.add_argument('--logprob_threshold', type=float, default=-1.0, help='跳过低对数概率样本阈值')
    parser.add_argument('--temperature', type=float, default=1.0, help='生成温度，必须大于0以免 None 引发错误，默认为1.0')
    parser.add_argument('--max_samples', type=int, default=100, help='测试模式下最大标注音频数，<=0 则不限制，默认为100')
    ...
    logger.info(f"设备: {'CPU' if args.device == -1 else f'GPU {args.device}'}")
    logger.info("=" * 60)
    ...
    for root, _, files in os.walk(args.audio_dir):
        for fname in files:
            ...
            try:
                ...
                try:
                    if cr > args.compression_ratio_threshold or lp < args.logprob_threshold:
                        logger.warning(f"低置信度，跳过: cr={cr:.2f}, lp={lp:.2f}, 文件: {audio_path}")
                        continue
                except TypeError as e:
                    # 记录错误日志，跳过置信度过滤
                    logger.warning(f"置信度比较出错，跳过过滤: cr={cr}, lp={lp}, error={e}, 文件: {audio_path}")
                    logger.debug(f"错误堆栈:\n{traceback.format_exc()}")
                # 继续进行文本提取
