...
def load_model_with_peft(model_path, base_model_path, device, logger):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"从模型目录加载处理器失败，尝试从基础模型加载...")
            if base_model_path:
                try:
...
def prepare_audio(path, processor, device, sampling_rate=16000, logger=None):
    """准备音频数据，带错误处理"""
    try:
        ...
        if sr != sampling_rate:
            if logger:
                logger.debug(f"重采样音频: {sr}Hz -> {sampling_rate}Hz ({path})")
            data = np.interp(
                np.linspace(0, len(data), int(len(data) * sampling_rate / sr)),
                np.arange(len(data)), data
            )
        
        inputs = processor(data, sampling_rate=sampling_rate, return_tensors='pt', return_attention_mask=True)
        return inputs.input_features.to(device), inputs.attention_mask.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, required=True, help='微调后模型目录')
    parser.add_argument('--base_model_path', type=str, default=None, help='基础模型路径（LoRA 模型需要）')
    parser.add_argument('--test_manifest', type=str, required=True, help='测试数据 CSV 清单，含 audio_filepath,text')
    parser.add_argument('--output_file', type=str, default='eval_results.txt', help='评估结果输出文件')
    parser.add_argument('--batch_size', type=int, default=1, help='批量大小，仅支持逐条或小批量')
    parser.add_argument('--metric', type=str, default='cer', choices=['wer','cer'], help='评估指标')
    parser.add_argument('--language', type=str, default='zh', help='识别语言，例如 zh 或 en')
    parser.add_argument('--task', type=str, default='transcribe', choices=['transcribe','translate'], help='任务类型：转写或翻译')
    parser.add_argument('--max_new_tokens', type=int, default=256, help='最大生成 token 数')
    parser.add_argument('--no_repeat_ngram_size', type=int, default=2, help='禁止重复 n-gram 大小')
    ...
    # 查找模型路径
    model_path, base_model_path, is_merged = find_model_path(args.model_dir, logger)
    ...
    model, processor = load_model_with_peft(model_path, base_model_path, device, logger)
    ...
    try:
        logger.info("加载测试数据集...")
        ds = load_dataset('csv', data_files={'test': args.test_manifest})['test']
    ...
    for idx, example in enumerate(ds):
        try:
            audio_fp = example.get('audio_filepath') or example.get('audio_file')
            if audio_fp is None:
            ...
            # 准备音频
            input_feats, attn_mask = prepare_audio(audio_fp, processor, device, logger=logger)
