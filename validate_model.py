...
def preprocess_text(text):
    ...
    # 保留中英文、百分号与空格
    text = "".join(
        ch for ch in text if re.match(r"[\u4e00-\u9fa5A-Za-z% ]", ch)
    )
    # 去除空格
    text = text.replace(" ", "")
    return text.strip()
...
def transcribe_xunfei(audio_fp, config_path):
    ...
    # 上传音频
    resp = requests.post(
        upload_url, headers=headers, data=audio_data, timeout=timeout
    )
    rj = resp.json()
    if rj.get("code") != "0":
        return ""
...
def main():
    ...
    parser.add_argument(
        "--manifest", required=True, help="验证集 manifest CSV 路径"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eval_metric", choices=["wer", "cer"], default="cer"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Beam 搜索大小， >1 时启用束搜索",
    )
    parser.add_argument(
        "--length_penalty", type=float, default=1.0, help="生成长度惩罚"
    )
    parser.add_argument(
        "--early_stopping",
        action="store_true",
        help="启用 early_stopping 以在达到 eos 时提前结束",
    )
    parser.add_argument(
    ...
    use_whisper = (
        True
        if "whisper" in args.base_model_name_or_path.lower()
        else False
    )
    processor, model = load_model(
        args.base_model_name_or_path, args.model_dir, use_whisper, device
    )
    # 加载原始基模型，用于速度对比
    if use_whisper:
        base_model = (
        ...
        base_model = (
            Wav2Vec2ForCTC.from_pretrained(args.base_model_name_or_path)
            .to(device)
            .eval()
        )
    # 初始化保存预测与时间列表
        ...
        for idx, row in enumerate(reader):
            ...
            with torch.no_grad(), sf.SoundFile(audio_fp) as f:
                for block in f.blocks(
                    ...
                    if use_whisper:
                        ...
                        base_txt = processor.batch_decode(
                        ...
                        lora_txt = processor.batch_decode(
                            lora_ids, skip_special_tokens=True
                        )[0]
                        ...
                        t_base_total += time.time() - t0
                        base_txt = processor.batch_decode(
                            ids_base, skip_special_tokens=True
                        )[0]
                        base_txt = preprocess_text(base_txt)
                        base_pred_segs.append(base_txt)
                        t1 = time.time()
                        ...
                        t_lora_total += time.time() - t1
                        lora_txt = processor.batch_decode(
                            ids_lora, skip_special_tokens=True
                        )[0]
                        lora_txt = preprocess_text(lora_txt)
                        lora_pred_segs.append(lora_txt)
                        # 清理临时变量并释放显存
            ...
            if use_whisper:
                ...
                base_pred = processor.batch_decode(
                    base_gen_ids, skip_special_tokens=True
                )[0]
                # 标准化基准模型输出
                ...
                t_lora = time.time() - t1
                lora_pred = processor.batch_decode(
                    lora_gen_ids, skip_special_tokens=True
                )[0]
                # 标准化LoRA模型输出
    ...
    # 打印速度统计示例
    print(
        f"Avg base time: {sum(base_times)/len(base_times):.3f}s, Avg LoRA time: {sum(lora_times)/len(lora_times):.3f}s"
    )
    # 输出结果CSV
    fieldnames = [
        "id",
