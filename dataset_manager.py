...
def check_data_size(data_size, dataset_name, logger):
    ...
    if data_size == 0:
        ...
        logger.warning(f"警告：{dataset_name} 数据集数据量很少（仅{data_size}条记录），建议至少10条以上")
...
def main():
    ...
    parser.add_argument('--labels_dir', type=str, required=True, help='标注文件目录')
    parser.add_argument('--output_dir', type=str, required=True, help='输出清单目录')
    ...
    is_first_iteration = not (os.path.exists(train_csv) and os.path.exists(val_csv) and os.path.exists(test_csv))
    ...
    if is_first_iteration:
        ...
        logger.info(f"初始分割完成：训练集 {len(train_data)} 条，验证集 {len(val_data)} 条，测试集 {len(test_data)} 条")
    else:
        logger.info("非首次迭代：将新数据添加到训练集")
        # 非首次迭代：读取现有清单，找出新数据并添加到训练集
        existing_train_paths, existing_train_data = read_existing_csv(train_csv)
        existing_val_paths, existing_val_data = read_existing_csv(val_csv)
        existing_test_paths, existing_test_data = read_existing_csv(test_csv)
        
        all_existing_paths = existing_train_paths | existing_val_paths | existing_test_paths
        
        logger.info(f"现有数据：训练集 {len(existing_train_data)} 条，验证集 {len(existing_val_data)} 条，测试集 {len(existing_test_data)} 条")
        ...
        logger.info(f"更新完成：训练集 {len(updated_train_data)} 条（新增 {len(new_pairs)} 条），验证集 {len(existing_val_data)} 条，测试集 {len(existing_test_data)} 条")
