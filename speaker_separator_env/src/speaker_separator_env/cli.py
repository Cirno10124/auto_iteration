...
def main():
    ...
    parser.add_argument(
        "--out_dir", type=str, default=None, help="保存分段 wav 的输出目录（默认不保存）"
    )
    parser.add_argument(
        "--cluster",
        action="store_true",
        help="是否额外做一次聚类（embedding + 层次聚类）",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=0.75,
        help="聚类距离阈值（cosine distance）",
    )
    ...
    if args.out_dir:
        out_dir = sep.save_speaker_segments(
            args.audio, out_dir=args.out_dir
        )
        print(f"[save] wrote speaker segments to: {out_dir}")

    if args.cluster:
        clusters = sep.cluster_speakers(
            args.audio, threshold=args.threshold
        )
